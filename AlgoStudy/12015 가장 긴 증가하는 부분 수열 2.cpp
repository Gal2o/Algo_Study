...
//        mid = (start + end) / 2;
//
//        if (answer[mid] < target) // 10 20 50 | target = 30, answer[mid] = 20
//            start = mid + 1;
//            // 넣어야 할 값이 answer[mid] 보다 크면 left = [mid + 1] ~ end 에서 다시 검색
//            // mid+1 = 3 이므로 20과 50사이에 target 이 추가 되는 것이 아니라 10 20 30으로 변경하고 while 탈출
//        else
//            end = mid; // 10 40 50 | target = 30, answer[mid] = 40
...
//    answer[1] = su[1]; // 시작은 1부터
//
////  1부터 n까지 su배열을 돌면서 answer[idx] < su[i] 면, answer[idx + 1] = su[i]
//    for (int i = 1; i <= n; i++) {
//        if (answer[idx] < su[i]) {
//            answer[idx + 1] = su[i];
//        // 다음 인덱스로 넘어가기
